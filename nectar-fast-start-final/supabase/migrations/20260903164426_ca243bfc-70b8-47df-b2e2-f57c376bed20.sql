ALTER TYPE public.device_status ADD VALUE IF NOT EXISTS 'pending_sale';
ALTER TYPE public.lead_status ADD VALUE IF NOT EXISTS 'pending';

CREATE OR REPLACE FUNCTION public.create_deal_with_terminal(p_lead_id uuid, p_deal_type deal_type, p_hardware_amount numeric, p_subscription_monthly numeric, p_subscription_months integer, p_notes text, p_payment_method text, p_device_id uuid, p_duration_days integer, p_is_demo boolean)
 RETURNS uuid
 LANGUAGE plpgsql
...
BEGIN
  ...
  UPDATE public.devices
     SET status = CASE
       ...
       ELSE 'pending_sale'::public.device_status
     END,
  ...
  INSERT INTO public.deals (
    ...
    v_user_id,
    p_deal_type,
    CASE WHEN v_is_contingent THEN 'open'::public.deal_status ELSE 'pending'::public.deal_status END,
    v_hardware_amount,
  ...
  ELSE
    UPDATE public.leads
       SET status = 'pending'::public.lead_status
     WHERE id = p_lead_id
       AND status NOT IN ('won'::public.lead_status, 'do_not_contact'::public.lead_status);
  END IF;
